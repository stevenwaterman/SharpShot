    ...
    override val root = vbox {
        ...
        hbox(4.0) {
            ...
            button("Equal to Zero") {
                action { acceptZero() }
                tooltip("Redirects bullet when their value is zero (Q)")
                isFocusTraversable = false
            }
            button("Over Zero") {
                action { acceptPositive() }
                tooltip("Redirects bullets when their value is larger than zero (W)")
                isFocusTraversable = false
            }
            button("Empty") {
                action { acceptEmpty() }
                tooltip("Redirects bullets that are empty (E)")
                isFocusTraversable = false
            }
        }
    }
