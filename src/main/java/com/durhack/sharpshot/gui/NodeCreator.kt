...
internal class NodeCreator(getDefaultInNodeIndex: () -> Int) : ListView<NodeTypeDescriptor<*>>(FXCollections.observableArrayList()) {
    init {
        items.addAll(
                listOf(
                        NodeTypeDescriptor(InNode(null), "Provides Input at program start and every time a bullet passes through") {
                            val index = getNumberInput("Enter Input Index", "Blank to shoot empty bullet at start\nArguments are 0-indexed", getDefaultInNodeIndex())?.toInt() ?: return@NodeTypeDescriptor null
                            return@NodeTypeDescriptor InNode(index)
                        },
                        NodeTypeDescriptor(OutNode(), "Consumes and prints bullets") { OutNode() },
                        NodeTypeDescriptor(AsciiNode(), "Consumes and prints bullets based on their ASCII code equivalent") { AsciiNode() },
                        ...
                        NodeTypeDescriptor(IfPositiveNode(), "Redirects all positive bullets (>0). Other bullets pass through unaffected") { IfPositiveNode() },
                        NodeTypeDescriptor(IfZeroNode(), "Redirects all zero bullets (=0). Other bullets pass through unaffected") { DivNode() },
                        NodeTypeDescriptor(SplitterNode(), "A bullet in one side produces 3 bullets in the others") { SplitterNode() },
                        NodeTypeDescriptor(ConstantNode(BigInteger.ZERO), "Whenever a bullet passes through, release another bullet with pre-set value") {
                            val value = getNumberInput("Enter Constant Value", "Leave empty for empty bullets", getDefaultInNodeIndex())
                            return@NodeTypeDescriptor ConstantNode(value)
                        },
                        NodeTypeDescriptor(RandomNode(), "Provides a random output from 0 (inclusive) to input bullet value (exclusive)") { SplitterNode() },
                        NodeTypeDescriptor(RotateNode(), "Rotates incoming bullets clockwise") { RotateNode() },
                        NodeTypeDescriptor(ACRotateNode(), "Rotates incoming bullets anticlockwise") { ACRotateNode() },
                        ...
                        NodeTypeDescriptor(ListNode(), "Every time a bullet comes in, outputs the next value in the list of inputs") { ListNode() },
                        NodeTypeDescriptor(StackNode(), "Inputs in the back pop from the stack, inputs to other sides get added to the stack") { StackNode() }
                        )
                    )
    }
