...
/**
 * THIS CODE IS SHIT BUT I JUST STOLE IT FROM SUN AND IT'S THE ONLY WAY TO CHANGE THE BEHAVIOUR OF SCROLL PANE SO IT DOESN'T KEEP CHANGING THE SCROLL BAR POSITIONS
 */
public class MyScrollPaneSkin extends BehaviorSkinBase<ScrollPane, ScrollPaneBehavior> implements TraverseListener {
    /***************************************************************************
     *                                                                         *
     * UI Subcomponents                                                        *
     *                                                                         *
    ...
    private final InvalidationListener nodeListener = new InvalidationListener() {
        ...
        public void invalidated(Observable valueModel) {
            if (!nodeSizeInvalid) {
                 ...
                 */
                if (vsbvis != determineVerticalSBVisible() || hsbvis != determineHorizontalSBVisible() || (scrollNodeWidth != 0.0 && nodeWidth != scrollNodeWidth) || (scrollNodeHeight != 0.0 && nodeHeight != scrollNodeHeight)) {
    ...
    private final ChangeListener<Bounds> boundsChangeListener = new ChangeListener<Bounds>() {
        ...
        public void changed(ObservableValue<? extends Bounds> observable, Bounds oldBounds, Bounds newBounds) {
            ...
            if (oldHeight > 0 && oldHeight != newHeight) {
                ...
                updateScrollBar(oldPositionY, newPositionY, vsb);
            }
            ...
            if (oldWidth > 0 && oldWidth != newWidth) {
                ...
                updateScrollBar(oldPositionX, newPositionX, hsb);
            }
        }
    };

    private void updateScrollBar(double oldPositionX, double newPositionX, ScrollBar hsb) {
...
//        setManaged(false);
        ...
        EventHandler<MouseEvent> barHandler = ev -> getSkinnable().requestFocus();
        ...
        viewContent = new StackPane() {
            ...
            public void requestLayout() {
                ...
                MyScrollPaneSkin.this.getSkinnable().requestLayout();
            }
        ...
        viewRect.addEventFilter(MouseEvent.MOUSE_RELEASED, e -> {
             ...
             */
            if ((posY > getSkinnable().getVmax() || posY < getSkinnable().getVmin() || posX > getSkinnable().getHmax() || posX < getSkinnable().getHmin()) && !touchDetected) {
        ...
        blockDispatch(blockEventDispatcher, hsb);
        // block ScrollEvent from being passed down to scrollbar's skin
        blockDispatch(blockEventDispatcher, vsb);
        ...
        viewRect.addEventHandler(ScrollEvent.SCROLL, event -> {
            ...
            if (vsb.getVisibleAmount() < vsb.getMax()) {
                ...
                if (!IS_TOUCH_SUPPORTED) {
                    if ((event.getDeltaY() > 0.0 && vsb.getValue() > vsb.getMin()) || (event.getDeltaY() < 0.0 && vsb.getValue() < vsb.getMax())) {
                     ...
                     */
                    setIfNotInProgress(event, newValue, vsb);
                }
            }
            ...
            if (hsb.getVisibleAmount() < hsb.getMax()) {
                ...
                if (!IS_TOUCH_SUPPORTED) {
                    if ((event.getDeltaX() > 0.0 && hsb.getValue() > hsb.getMin()) || (event.getDeltaX() < 0.0 && hsb.getValue() < hsb.getMax())) {
                     ...
                     ** set the value for 'real' events
                     */
                    setIfNotInProgress(event, newValue, hsb);
                }
            }
        });
    ...
    private void setIfNotInProgress(ScrollEvent event, double newValue, ScrollBar vsb) {
        if (!event.isInertia() || contentsToViewTimeline == null || contentsToViewTimeline.getStatus() == Status.STOPPED) {
            vsb.setValue(newValue);
    ...
    private void blockDispatch(EventDispatcher blockEventDispatcher, ScrollBar hsb) {
    ...
    private void updateScrollBar(double newHVal, ScrollBar hsb) {
        if (!IS_TOUCH_SUPPORTED) {
    ...
    private void scrollBoundsIntoView(Bounds b) {
        double dx = 0.0;
        double dy = 0.0;
        ...
        updateScrollBar(dx, hsb, nodeWidth, contentWidth);
        updateScrollBar(dy, vsb, nodeHeight, contentHeight);

    }

    private void updateScrollBar(double dx, ScrollBar hsb, double nodeSize, double contentsSize) {
        if (dx != 0) {
            double sdx = dx * (hsb.getMax() - hsb.getMin()) / (nodeSize - contentsSize);
    ...
    private double computeHsbSizeHint(ScrollPane sp) {
        return ((sp.getHbarPolicy() == ScrollBarPolicy.ALWAYS) || (sp.getHbarPolicy() == ScrollBarPolicy.AS_NEEDED && (sp.getPrefViewportHeight() > 0 || sp.getMinViewportHeight() > 0))) ?
                hsb.prefHeight(
                ScrollBar.USE_COMPUTED_SIZE) : 0;
    }
    ...
    private double computeVsbSizeHint(ScrollPane sp) {
        return ((sp.getVbarPolicy() == ScrollBarPolicy.ALWAYS) || (sp.getVbarPolicy() == ScrollBarPolicy.AS_NEEDED && (sp.getPrefViewportWidth() > 0 || sp.getMinViewportWidth() > 0))) ? vsb.prefWidth(
                ScrollBar.USE_COMPUTED_SIZE) : 0;
    }
    ...
    protected void layoutChildren(final double x, final double y, final double w, final double h) {
        ...
        if (vsbvis) {
             ...
             */
            vsb.resizeRelocate(snappedLeftInset() + w - vsbWidth + (rightPadding < 1 ? 0 : rightPadding - 1), cy, vsbWidth, vsbHeight);
        }
        updateVerticalSB();
        ...
        if (hsbvis) {
             ...
             **   If padding is < 1 then we just stick to the edge.
             */
            hsb.resizeRelocate(cx, snappedTopInset() + h - hsbHeight + (bottomPadding < 1 ? 0 : bottomPadding - 1), hsbWidth, hsbHeight);
        }
        updateHorizontalSB();
    ...
    private void computeScrollNodeSize(double contentWidth, double contentHeight) {
        if (scrollNode != null) {
            if (scrollNode.isResizable()) {
                ...
                if (bias == null) {
                    nodeWidth = snapSize(boundedSize(control.isFitToWidth() ? contentWidth : scrollNode.prefWidth(-1), scrollNode.minWidth(-1), scrollNode.maxWidth(-1)));
                    nodeHeight = snapSize(boundedSize(control.isFitToHeight() ? contentHeight : scrollNode.prefHeight(-1), scrollNode.minHeight(-1), scrollNode.maxHeight(-1)));

                }
                else if (bias == Orientation.HORIZONTAL) {
                    nodeWidth = snapSize(boundedSize(control.isFitToWidth() ? contentWidth : scrollNode.prefWidth(-1), scrollNode.minWidth(-1), scrollNode.maxWidth(-1)));
                    nodeHeight = snapSize(boundedSize(control.isFitToHeight() ? contentHeight : scrollNode.prefHeight(nodeWidth), scrollNode.minHeight(nodeWidth), scrollNode.maxHeight(nodeWidth)));

                }
                else { // bias == VERTICAL
                    nodeHeight = snapSize(boundedSize(control.isFitToHeight() ? contentHeight : scrollNode.prefHeight(-1), scrollNode.minHeight(-1), scrollNode.maxHeight(-1)));
                    nodeWidth = snapSize(boundedSize(control.isFitToWidth() ? contentWidth : scrollNode.prefWidth(nodeHeight), scrollNode.minWidth(nodeHeight), scrollNode.maxWidth(nodeHeight)));
                }
    ...
    private boolean isReverseNodeOrientation() {
        return (scrollNode != null && getSkinnable().getEffectiveNodeOrientation() != scrollNode.getEffectiveNodeOrientation());
    }
    ...
    private boolean determineHorizontalSBVisible() {
        ...
        if (IS_TOUCH_SUPPORTED) {
            ...
            return (ScrollBarPolicy.NEVER != hbarPolicy) && ((ScrollBarPolicy.ALWAYS == hbarPolicy) || (((sp.isFitToWidth() && scrollNode != null) && scrollNode.isResizable()) ?
                    (nodeWidth > contentWidth && scrollNode
    ...
    private boolean determineVerticalSBVisible() {
        ...
        if (IS_TOUCH_SUPPORTED) {
            ...
            return (ScrollBarPolicy.NEVER != vbarPolicy) && ((ScrollBarPolicy.ALWAYS == vbarPolicy) || (((sp.isFitToHeight() && scrollNode != null) && scrollNode.isResizable()) ?
                    (nodeHeight > contentHeight && scrollNode
                    .minHeight(-1) > contentHeight) : (nodeHeight > contentHeight)));
        }
    }
    ...
    private void updateHorizontalSB() {
        updateSB(nodeWidth, hsb, contentWidth);
    ...
    private void updateSB(double nodeSize, ScrollBar hsb, double contentsSize) {
        double contentRatio = nodeSize * (hsb.getMax() - hsb.getMin());
        if (contentRatio > 0.0) {
            hsb.setVisibleAmount(contentsSize / contentRatio);
    ...
    private void updateVerticalSB() {
        updateSB(nodeHeight, vsb, contentHeight);
    ...
    private Timeline contentsToViewTimeline;
    ...
    private void startSBReleasedAnimation() {
        if (sbTouchTimeline == null) {
            ...
            KeyFrame sbTouchKF1 = new KeyFrame(Duration.millis(0), event -> {
                tempVisibility = true;
                if (touchDetected || mouseDown) {
                    sbTouchTimeline.playFromStart();
                }
            });

            KeyFrame sbTouchKF2 = new KeyFrame(Duration.millis(1000), event -> {
    ...
    private void startContentsToViewport() {
         ...
         */
        KeyFrame contentsToViewKF1 = new KeyFrame(Duration.millis(50));
        /*
         ** reposition
         */
        KeyFrame contentsToViewKF2 = new KeyFrame(Duration.millis(150), event -> getSkinnable().requestLayout(), new KeyValue(contentPosX, newPosX), new KeyValue(contentPosY, newPosY));
        /*
         ** block out 'aftershocks', but real events will
         ** still reactivate
         */
        KeyFrame contentsToViewKF3 = new KeyFrame(Duration.millis(1500));
